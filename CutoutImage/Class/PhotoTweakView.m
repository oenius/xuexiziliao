...
static const NSUInteger kGridLines = 7;
             ...
             maxRotationAngle:(CGFloat)maxRotationAngle
{
    if (self = [super init]) {
        ...
        _centerY = self.maximumCanvasSize.height / 2 + kCanvasHeaderHeigth*2;
        ...
        UILabel * sliderLabel = [[UILabel alloc]initWithFrame:CGRectMake(0, 10, self.bounds.size.width/4, 30)];
        [self addSubview:sliderLabel];
        sliderLabel.textAlignment = NSTextAlignmentCenter;
        sliderLabel.adjustsFontSizeToFitWidth = YES;
        sliderLabel.text = CI_Rotate;
        sliderLabel.textColor = [UIColor whiteColor];
        
        _slider = [[UISlider alloc] initWithFrame:CGRectMake( self.bounds.size.width/4,10, self.bounds.size.width/4*3-10, 30)];
...
//        _resetBtn.frame = CGRectMake(0, 0, CGRectGetWidth(self.bounds) / 3, 20);
//        _resetBtn.center = CGPointMake(CGRectGetWidth(self.bounds) / 2, CGRectGetHeight(self.bounds) - 60);
...
- (void)sliderValueChanged:(UIRotationGestureRecognizer*)rotate
{
    // update masks
    [self updateMasks:NO];
