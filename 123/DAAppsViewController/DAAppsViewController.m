...
{
    [self loadAppsWithPath:path defaultTitle:defaultTitle exceptAppIds:nil completionBlock:block];
}

- (void)loadAppsWithPath:(NSString *)path defaultTitle:(NSString *)defaultTitle exceptAppIds:(NSArray *)exceptAppIds completionBlock:(void(^)(BOOL result, NSError *error))block
{
    _isLoading = YES;
    ...
    [self loadRequestPath:path withCompletion:^(NSArray *results, NSError *error) {
        ...
        if (error) {
            ...
            for (NSDictionary *result in results) {
                ...
                BOOL shouldExcept = NO;
                if (exceptAppIds.count > 0) {
                    for (NSString *exceptAppId in exceptAppIds) {
                        NSInteger appId = exceptAppId.integerValue;
                        if (appObject.appId == appId) {
                            shouldExcept = YES;
                        }
                    }
                }
                if (appObject && ![mutableApps containsObject:appObject] && !shouldExcept) {
...
- (void)loadAppsWithArtistId:(NSInteger)artistId exceptAppIds:(NSArray *)exceptAppIds completionBlock:(void(^)(BOOL result, NSError *error))block
{
    NSString *requestPath = [NSString stringWithFormat:@"lookup?id=%ld", (long)artistId];
//    [self loadAppsWithPath:requestPath defaultTitle:NSLocalizedString(@"Results",) completionBlock:block];
    [self loadAppsWithPath:requestPath defaultTitle:NSLocalizedString(@"Results",) exceptAppIds:exceptAppIds completionBlock:block];
}
