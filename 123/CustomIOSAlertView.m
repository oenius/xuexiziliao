...
- (void)show
{
    ...
    // Attached to the top most window
        ...
        UIWindow *rootVCWindow = [UIApplication sharedApplication].delegate.window;
        [rootVCWindow addSubview:self];
//        [[[[UIApplication sharedApplication] windows] firstObject] addSubview:self];
    }

    dialogView.layer.opacity = 0.5f;
//    dialogView.layer.transform = CATransform3DMakeScale(1.3f, 1.3f, 1.0);
    CGRect destFrame = dialogView.frame;
    CGRect startFrame = CGRectMake(destFrame.origin.x, 0, destFrame.size.width, destFrame.size.height);
...
//        dialogView.layer.transform = CATransform3DConcat(rotation, CATransform3DMakeScale(1, 1, 1));
    ...
    dialogView.layer.opacity = 1.0f;
    CGRect startFrame = dialogView.frame;
    CGRect destFrame = CGRectMake(startFrame.origin.x, 0, startFrame.size.width, startFrame.size.height);
...
- (UIView *)createContainerView
{
    ...
    if (titleHeight > 0) {
        CGSize contenerViewSize = containerView.frame.size;
        containerView.frame = CGRectMake(0, titleHeight, contenerViewSize.width, contenerViewSize.height);
    }
...
- (void)resetContainerView:(UIView *)newView{
...
- (void)addButtonsToView: (UIView *)container
{
    ...
    for (int i=0; i<[buttonTitles count]; i++) {
        ...
        if (i < ([buttonTitles count] -1)) {
            UIView *lineView = [[UIView alloc] initWithFrame:CGRectMake(closeButton.frame.size.width + closeButton.frame.origin.x, container.bounds.size.height - buttonHeight, buttonSpacerHeight , buttonHeight)];
            lineView.backgroundColor = [UIColor colorWithRed:198.0/255.0 green:198.0/255.0 blue:198.0/255.0 alpha:1.0f];
