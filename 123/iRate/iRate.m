...
// 是否获取到购买时的价格
#define keyHasGetPriceWhenInstall       @"keyHasGetPriceWhenInstall"
// 购买时的价格
#define keyPriceWhenInstall             @"keyPriceWhenInstall"
...
static NSString *const iRateCurrentLaunchedKey = @"iRateCurrentLaunched";
...
{
    return _cancelButtonLabel ?: [self localizedStringForKey:iRateRemindButtonKey withDefault:@"Remind Me Later"];
    //[self localizedStringForKey:iRateCancelButtonKey withDefault:@"No, Thanks"];
}
...
- (NSDate *)currentLaunchedTime {
    NSDate *launchTime = [[NSUserDefaults standardUserDefaults] objectForKey:iRateCurrentLaunchedKey];
    if (launchTime) {
        return launchTime;
    }else {
        return [NSDate date];
...
- (BOOL)hasGetPriceWhenInstall{
    NSUserDefaults *userDefault = [NSUserDefaults standardUserDefaults];
    BOOL hasGetPrice = [userDefault boolForKey:keyHasGetPriceWhenInstall];
    if (hasGetPrice) {
        return YES;
    }
    return NO;
...
- (void)setHasGetPriceWhenInstall:(BOOL)hasGetPrice {
    NSUserDefaults *userDefault = [NSUserDefaults standardUserDefaults];
    [userDefault setBool:hasGetPrice forKey:keyHasGetPriceWhenInstall];
...
- (CGFloat)priceWhenInstall{
    NSUserDefaults *userDefault = [NSUserDefaults standardUserDefaults];
    CGFloat price = [userDefault floatForKey:keyPriceWhenInstall];
    return price;
}

- (void)setPriceWhenInstall:(CGFloat)price {
    NSUserDefaults *userDefault = [NSUserDefaults standardUserDefaults];
    [userDefault setFloat:price forKey:keyPriceWhenInstall];
    [userDefault synchronize];
}
            ...
            if (!error)
            {
                ...
                if (bundleID)
                {
                    if ([bundleID isEqualToString:self.applicationBundleID])
                    {
                        ...
                        self.appStoreTrackName = trackName;
                        
                        BOOL hasGetPrice = [self hasGetPriceWhenInstall];
                        if (NO == hasGetPrice) {
                            NSString *price = [self valueForKey:@"price" inJSON:json];
                            CGFloat priceFloat = price.floatValue;
                            [self setPriceWhenInstall:priceFloat];
                            dispatch_async(dispatch_get_main_queue(), ^(void) {
                                [[NSNotificationCenter defaultCenter] postNotificationName:kGetPriceWhenInstallNotification object:nil];
                            });

                            [self setHasGetPriceWhenInstall:YES];
                        }
...
- (void)promptForRating
{
    if (!self.visibleAlert)
    {
        ...
        if ([UIAlertController class] && topController && self.useUIAlertControllerIfAvailable)
        {
            ...
            {
                [alert addAction:[UIAlertAction actionWithTitle:self.remindButtonLabel style:UIAlertActionStyleCancel handler:^(__unused UIAlertAction *action) {
...
- (void)applicationLaunched
{
    ...
    [defaults setObject:[NSDate date] forKey:iRateCurrentLaunchedKey];
