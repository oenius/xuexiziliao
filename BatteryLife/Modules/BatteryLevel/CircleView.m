...
#define DEGREES_DefaultStart(degrees)  ((M_PI * (degrees+270))/180) + M_PI_2
...
- (void)setupDefault
{
    _percentText = @"0%";
    _lineWidth = 25;
    _cirleColor = [UIColor greenColor];
    _percentFont = [UIFont fontWithName:@"Arial"size:27];
    _animationTime = 1.5;
}
...
-(void)drawBackCircle{
    ...
    [edgePath addArcWithCenter:CGPointMake(self.frame.size.width*0.5, self.frame.size.height * 0.5) radius:radius * 0.5 startAngle:0 endAngle:2*M_PI clockwise:YES];
    if(!self.animtionLayer){
        ...
        self.edgeLayer.strokeColor = [UIColor whiteColor].CGColor;
...
-(void)addPercentCircle{
    ...
    CGFloat radius = MIN(self.bounds.size.width, self.bounds.size.height);
    [animationPath addArcWithCenter:CGPointMake(self.bounds.size.width*0.5, self.bounds.size.height * 0.5) radius:radius * 0.5 startAngle:DEGREES_DefaultStart(0) endAngle:self.endDegree clockwise:YES];
...
- (void)drawRect:(CGRect)rect
{
    ...
    self.percentFont = [self.percentFont fontWithSize:self.frame.size.width/4.5];
