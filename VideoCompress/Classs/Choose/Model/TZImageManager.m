...
NSString *const kAssetFilesizeKey       = @"kAssetFilesizeKey";
NSString *const kAssetThumImageKey      = @"kAssetThumImageKey";
NSString *const kAssetPixelWidthKey     = @"kAssetPixelWidthKey";
NSString *const kAssetPixelHeightKey    = @"kAssetPixelHeightKey";
NSString *const kAssetBitrateKey        = @"kAssetBitrateKey";
NSString *const kAssetDurationKey       = @"kAssetDurationKey";
NSString *const kAssetCreationDate      = @"kAssetCreationDate";
NSString *const kAssetAVAssetKey        = @"kAssetAVAssetKey";
...
/// 获得asset的详细信息(获得视频大小和比特率)
-(void)getVideoAssetBirteAndSize:(id)assset completion:(void (^)(NSDictionary *details))completion{
    TZAssetModel *model = assset;
    if ([model.asset isKindOfClass:[PHAsset class]]) {
...
//                    NSLog(@"bitelu:%g",bitRate);
//                }
//                estimatedSize += track.totalSampleDataLength;
//            }
            NSDictionary * details = @{kAssetAVAssetKey:asset};
...
/// 获得asset缩略图和宽度高度等其他信息
-(void)getVideoAssetThumImageAndOther:(id)asset completion:(void (^)(NSDictionary *details))completion{
    TZAssetModel *model = asset;
    if ([model.asset isKindOfClass:[PHAsset class]]) {
        PHAsset * phasset = (PHAsset *)model.asset;
        [[PHImageManager defaultManager] requestImageForAsset:model.asset targetSize:CGSizeMake(200, 200) contentMode:PHImageContentModeAspectFill options:nil resultHandler:^(UIImage * _Nullable result, NSDictionary * _Nullable info) {
          
            NSDictionary * details = @{kAssetThumImageKey:result,
                                       kAssetPixelWidthKey:[NSNumber numberWithInteger:phasset.pixelWidth],
                                       kAssetPixelHeightKey:[NSNumber numberWithInteger:phasset.pixelHeight],
                                       kAssetDurationKey:[NSNumber numberWithFloat:phasset.duration],
                                       kAssetCreationDate:phasset.creationDate
                                       };
            if (completion) completion(details);
        }];
...
- (void)getPostImageWithAlbumModel:(TZAlbumModel *)model completion:(void (^)(UIImage *))completion {
    if (iOS8Later) {
        ...
        [[TZImageManager shareInstance] getPhotoWithAsset:asset photoWidth:80 completion:^(UIImage *photo, NSDictionary *info, BOOL isDegraded) {
